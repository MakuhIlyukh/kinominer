...
class Callback:
    ...
    парсинга. Parser хранит список Callback'ов в поле callbacks.
    Передав в этот список объект наследника класса Callback, вы дадите
    возможность парсеру вызвать этот метод при прохождении указанного
    этапа.
    ...
    За описанием событий обратитесь к документации соответствующих
    методов

    Последовательность вызовов callback'ов
    --------------------------------------

    Здесь представлен псевдокод, который изображает хронологию вызовов
    callback'ов:
    ...
    def on_item_end_ok(self, parser, item_url, item):
        '''
        Вызывается после парсинга всех страниц, соответствующих
        элементу. Например, если парсятся страницы оценок и друзей
        юзера, то вызовется после завершения парсинга страниц
        (один раз для каждого юзера)
    ...
    def on_item_end_finally(self, parser, item_url, item):
        '''
        Отличается от on_item_end_ok тем, что выполняется даже
        если было сгенерировано исключение.
        '''
        pass
    ...
    def on_item_error(self, parser, item_url, item, exc):
        ...
        Для обработки исключения возникновения капчи используйте
        on_captcha_error. 
        '''
        pass
...
def do_callbacks(callbacks, method_name, parser,
                 item_url=None, item=None, exc=None):
